...
describe('activeTeams', () => {
  it('collects real teams from unplayed and live matches only', () => {
    const teams = activeTeams([
      m({ stage: 'SF', t1: 'Argentina', t2: 'Canada' }), // unplayed → both active
      m({ stage: 'SF', t1: 'Uruguay', t2: 'Colombia', score: [1, 0], live: { minute: 70 } }), // live → active
      m({ stage: 'QF', t1: 'Brazil', t2: 'Panama', score: [2, 0] }), // final → not active
      m({ stage: 'Final', t1: 'Winner Match 29', t2: 'Winner Match 30' }), // placeholders → skipped
      m({ t1: 'Peru', t2: 'Chile', voided: true }), // voided → skipped
    ])
    expect([...teams].sort()).toEqual(['Argentina', 'Canada', 'Colombia', 'Uruguay'])
  })
