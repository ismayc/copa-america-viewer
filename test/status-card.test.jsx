...
const base = MATCHES.find((m) => m.num === 1) // Argentina v Canada (Group A)
...
describe('MatchCard follow / clinch / slot tooltip', () => {
  it('toggles follow', () => {
    renderCard()
    fireEvent.click(screen.getByRole('button', { name: 'Follow Argentina' }))
    expect(screen.getByRole('button', { name: 'Unfollow Argentina' })).toBeInTheDocument()
  })

  it('won-group clinch badge + slot tooltip', () => {
    renderCard(base, { clinch: { Argentina: 'won-group' } })
    expect(screen.getByText(/Won group/)).toBeInTheDocument()
    expect(screen.getByText('Argentina').getAttribute('title')).toMatch(/Clinched Group A winner/)
  })

  it('eliminated slot tooltip', () => {
    renderCard(base, { clinch: { Argentina: 'eliminated' } })
    expect(screen.getByText('Argentina').getAttribute('title')).toMatch(/Eliminated from Group A/)
  })
