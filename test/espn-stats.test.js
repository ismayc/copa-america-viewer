...
describe('fetchBootExtras', () => {
  ...
  it('serves from the localStorage cache within the TTL', async () => {
    ...
    const c = JSON.parse(localStorage.getItem('copa:bootExtras'))
    localStorage.setItem('copa:bootExtras', JSON.stringify({ ...c, at: Date.now() - CACHE_TTL_MS - 1 }))
    await fetchBootExtras()
...
describe('fetchRecentPlayerStats', () => {
  ...
  it('trusts a cached per-match line, skips an appearance-less log item, and survives unreadable cache', async () => {
    ...
    localStorage.setItem('copa:matchStat:700:45843', JSON.stringify({ a: 9, m: 9 }))
    ...
    localStorage.setItem('copa:matchStat:700:219713', 'not json')
    const seen = []
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      seen.push(url)
