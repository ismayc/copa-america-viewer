...
describe('App renders (smoke test)', () => {
  ...
  it('opens the panel, then the search, and filters with a scoped query', () => {
    ...
    fireEvent.change(input, { target: { value: 'team: Jamaica' } })
    // Jamaica went out in the group stage, so its 3 group games are its whole
    // tournament — a knockout side would also match its bracket ties.
  ...
  it('shows an active-filter count and "Clear all" when a filter is applied', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/team: Mexico/), {
      target: { value: 'team: Bolivia' },
    })
    expect(screen.getByRole('button', { name: /Clear all/ })).toBeInTheDocument()
    // Clearing resets results back to all 32 matches.
  ...
  it('shows past days folded by default, expandable per-day, and hideable entirely', () => {
    // Pin "now" mid-tournament so the June 20 opener is firmly in the past.
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2024-06-28T16:00:00Z'))
    try {
      render(<App />)
      ...
      const opener = screen.getByRole('button', { name: /June 20, 2024/ })
  ...
  it('keeps today and future days expanded by default', () => {
    ...
    try {
      render(<App />)
      // The final (July 14, 2024) is in the future — its day starts open.
...
describe('Standings clinch badges', () => {
  it('renders the clinch verdict next to a team when provided', () => {
    const clinch = { Argentina: 'won-group', Bolivia: 'eliminated' }
    render(
      <FollowProvider>
...
describe('Schedule team-name slot tooltip', () => {
  const groupMatch = MATCHES.find((m) => m.num === 1) // Argentina v Canada (Group A)
  ...
  it('shows the conditional knockout route when undecided', () => {
    renderCard({})
    const title = screen.getByText('Argentina').getAttribute('title')
    expect(title).toMatch(/Group A knockout route/)
    expect(title).toMatch(/1st → Quarter-final · Match 25/)
    expect(title).toMatch(/2nd → Quarter-final · Match 26/)
    // Copa has no best-third route, so the third line is a dead end, not a
  ...
  it('shows the definite slot once the group winner is clinched', () => {
    renderCard({ Argentina: 'won-group' })
    expect(screen.getByText('Argentina').getAttribute('title')).toBe(
      'Clinched Group A winner → Quarter-final · Match 25',
    )
  })
})
...
describe('Bracket clinch resolution', () => {
  it('renders the clinched winner once slots are resolved in the match data', () => {
    ...
    const resolved = resolveClinchedSlots(MATCHES, { Argentina: 'won-group' })
    render(
      <FollowProvider>
        <DetailContext.Provider value={() => {}}>
    ...
    // M25's first side was "Winner Group A" — now resolved to Argentina.
