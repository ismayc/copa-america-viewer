...
const matches = [
  ...
  },
  {
    ...
    t2: 'Ecuador',
    score: [1, 1],
    goals: { t1: [{ name: 'Raúl Jiménez' }], t2: [{ name: 'Enner Valencia' }] },
  },
]
...
describe('StatsView', () => {
  it('renders totals and the Golden Boot table', () => {
    ...
    expect(screen.getByText('Shamar Nicholson')).toBeInTheDocument()
  ...
  it('adds assists/minutes columns and award ordering once ESPN extras load', async () => {
    // Nicholson (1 goal) gets an assist edge over Valencia (1 goal, 0 assists).
    fetchBootExtras.mockImplementation(async () => [
      { name: 'Raúl Jiménez', goals: 3, assists: 0, minutes: 180 },
      { name: 'Shamar Nicholson', goals: 1, assists: 3, minutes: 90 },
      { name: 'Enner Valencia', goals: 1, assists: 0, minutes: 90 },
    ])
    render(<StatsView matches={matches} hideScores={false} />)
    ...
    // Award order: Jiménez (3g), then Nicholson above Valencia on assists; all get ranks
    // (no shared rank — the full key differs).
    expect(rows[0]).toHaveTextContent('Raúl Jiménez')
    expect(rows[1]).toHaveTextContent('Shamar Nicholson')
    expect(rows[1].cells[0]).toHaveTextContent('2')
    expect(rows[2]).toHaveTextContent('Enner Valencia')
    expect(rows[2].cells[0]).toHaveTextContent('3')
    // Nicholson's row shows his 3 assists and 90 minutes.
  ...
  it('bolds scorers whose team still has football to play', () => {
    ...
    const withRemaining = [...matches, { num: 29, stage: 'SF', t1: 'Mexico', t2: 'Colombia' }]
    render(<StatsView matches={withRemaining} hideScores={false} />)
    const rows = screen.getAllByRole('row').slice(1)
    const jimenez = rows.find((r) => r.textContent.includes('Raúl Jiménez'))
    const nicholson = rows.find((r) => r.textContent.includes('Shamar Nicholson'))
  ...
  it('overrides the lagging aggregate assists + minutes for a live match', async () => {
    ...
    const live = [
      ...matches,
      { num: 29, stage: 'SF', t1: 'Mexico', t2: 'Colombia', score: [0, 0], live: { minute: 80 }, espnId: '999', ko: recentKo(), goals: { t1: [], t2: [] } },
    ]
  ...
  it('clicking a player opens their match-by-match popup', () => {
    render(<StatsView matches={matches} hideScores={false} />)
    fireEvent.click(screen.getByRole('button', { name: 'Shamar Nicholson' }))
    expect(screen.getByRole('dialog', { name: /Shamar Nicholson — match by match/ })).toBeInTheDocument()
    fireEvent.click(screen.getByLabelText('Close'))
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  })

  it('marks players whose team is in a live match with an in-action dot', () => {
    const live = [...matches, { num: 29, stage: 'SF', t1: 'Mexico', t2: 'Colombia', score: [0, 0], live: { minute: 10 }, goals: { t1: [], t2: [] } }]
    render(<StatsView matches={live} hideScores={false} />)
    const rows = screen.getAllByRole('row').slice(1)
    const jimenez = rows.find((r) => r.textContent.includes('Raúl Jiménez'))
    const nicholson = rows.find((r) => r.textContent.includes('Shamar Nicholson'))
...
describe('StatsView tile drill-down', () => {
  const knockout = [
    ...matches,
    { num: 27, stage: 'QF', t1: 'Uruguay', t2: 'Brazil', ko: '2024-07-06T18:00:00-04:00', score: [2, 1], aet: true },
    { num: 28, stage: 'QF', t1: 'Colombia', t2: 'Panama', ko: '2024-07-06T21:00:00-04:00', score: [1, 1], aet: true, pens: [4, 2] },
  ]
  ...
  it('expands the extra-time tile into its match list (shootout noted)', () => {
    ...
    expect(screen.getByText(/Uruguay 2–1 Brazil/)).toBeInTheDocument()
    expect(screen.getByText(/Colombia 1–1 Panama/)).toBeInTheDocument()
  ...
  it('expands shootouts separately, and a row opens the match detail', () => {
    ...
    expect(screen.queryByText(/Uruguay 2–1 Brazil/)).not.toBeInTheDocument() // ET-only tie stays out
    fireEvent.click(screen.getByText(/Colombia 1–1 Panama/))
    expect(openDetail).toHaveBeenCalledWith(expect.objectContaining({ num: 28 }))
  })
...
describe('StatsView — enrichment that lands late, lands empty, or names a stranger', () => {
  const liveBoard = () => [
    ...matches,
    { num: 61, stage: 'SF', t1: 'Mexico', t2: 'Colombia', score: [0, 0], live: { minute: 10 }, goals: { t1: [], t2: [] } },
  ]
  ...
  it('folds an interval refresh that finally returns something into the table', async () => {
    ...
    try {
      ...
      fetchBootExtras.mockImplementation(async () => [
        ...
        { name: 'Shamar Nicholson', goals: 1, assists: 0, minutes: 90 },
      ])
      await act(async () => {
        await vi.advanceTimersByTimeAsync(5 * 60 * 1000 + 1000)
      })
  ...
  it('treats an empty reconciliation as no override at all', async () => {
    ...
    const recent = [
      ...matches,
      { num: 62, stage: 'SF', t1: 'Mexico', t2: 'Colombia', ko: recentKo(), score: [1, 0], espnId: 'e62', goals: { t1: [{ name: 'Raúl Jiménez' }], t2: [] } },
    ]
    render(<StatsView matches={recent} hideScores={false} />)
    await vi.waitFor(() => expect(fetchRecentPlayerStats).toHaveBeenCalled())
    const row = screen.getByText('Raúl Jiménez').closest('tr')
