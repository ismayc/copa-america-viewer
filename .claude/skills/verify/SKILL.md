...
`/copa-america-viewer/` prefix is needed in dev.
...
```
FEED  = https://raw.githubusercontent.com/openfootball/copa-america/master/2024--usa/copa.txt
ESPN  = https://site.api.espn.com/apis/site/v2/sports/soccer/conmebol.america/scoreboard
```

This feed is PLAIN TEXT, not JSON: `results.js` fetches it with `.text()` and runs
`parseCupTxt`. Fulfilling with `{ json: ... }` breaks it. It also carries no match
numbers, so the app matches feed rows to the schedule by kickoff date plus the
(order-independent) team pair.

## Pin the feeds for a deterministic run

Copa América 2024 is finished, so the committed board already carries every result and
...
```js
// copa-america-viewer's results feed is TEXT. Fulfil it as text, not JSON.
...
- **Champion**: `.champ-banner` (+ `.confetti`), `.nm-champ-runner`. Argentina won
  Copa América 2024, so the banner is present on a fresh load and reads "Argentina are the 2024 Copa América champions!".
...
- **`Scenarios` and the `Outlook` tab are gone.** They are group-phase tools and hide
  ...
  wait on them.
- This edition has 32 matches and the Final is match 32. Match numbers from
