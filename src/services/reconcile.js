// Source-agnostic score cross-check across our independent feeds:
//   • OpenFootball (record)  • ESPN (live)
//
// Source-agnostic is the point: it compares however many sources it is handed,
// which is what let this app drop from the family's usual three to two without
// touching a line here (TheSportsDB has no Copa América data — see
// services/results.js).
//
// Each source exposes a getter `score(match) -> { home, away, ft: [hg, ag] }`
// (or null when that source has no FINAL score for the match) — see
// openFootballFinalScore / espnFinalScore. This module only compares; it knows
// nothing about any source's wire format.
//
