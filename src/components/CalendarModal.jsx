...
const PROD = 'https://copa-america-viewer.netlify.app'
      ...
      <div className="md-card cal-modal" ref={cardRef} tabIndex={-1} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="md-section">
          ...
          <div className="cal-downloads">
            <button onClick={() => downloadICSCollection(matches, 'copa-america-2024-all.ics')}>
              All matches ({matches.length})
            </button>
            <button onClick={() => downloadICSCollection(filtered, 'copa-america-2024-filtered.ics')}>
              Current filter ({filtered.length})
            </button>
            {count > 0 && (
              <button onClick={() => downloadICSCollection(myMatches, 'copa-america-2024-my-teams.ics', 'Copa América 2024 — My Teams')}>
