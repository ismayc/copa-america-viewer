...
// A circular ("radial") rendering of the knockout bracket: the 8 quarter-finalists
// sit on the outer ring and each match's winner advances one ring inward, round
// by round, toward the trophy at the centre — so the flags progress along the
// spokes as results land. The third-place play-off sits just below the centre.
//
// Copa América's bracket is two rounds shallower than the World Cup's and one
// shallower than the Euro's: 8 outer slots and only two winner rings (QF, SF),
// so the rings are spread further apart than in either sibling.
...
const CX = 500
const CY = 500
const LEAF_R = 470 // outer ring: the 8 quarter-finalists
const RING = { QF: 330, SF: 165 } // winner of each round, inward
const SLOTS = 8
...
// The 8 outer slots, clockwise from just right of top: the right half of the
...
const LEAF_SLOTS = [
  ...BRACKET.right.QF.flatMap((n) => [{ n, side: 0 }, { n, side: 1 }]),
  ...[...BRACKET.left.QF].reverse().flatMap((n) => [{ n, side: 1 }, { n, side: 0 }]),
]
const slotAngle = (i) => 90 - (i + 0.5) * (360 / SLOTS)

// Angle of every match node: a QF node is the midpoint of its two outer slots;
...
function buildAngles() {
  ...
  for (const n of [...BRACKET.left.QF, ...BRACKET.right.QF]) {
    angle[n] = (leafAngleOf[`${n}:0`] + leafAngleOf[`${n}:1`]) / 2
  }
  for (const round of ['SF']) {
...
const ROUND_RING_LABELS = [
  { text: 'QUARTER-FINALS', r: (LEAF_R + RING.QF) / 2, size: 11 },
  { text: 'SEMI-FINALS', r: (RING.QF + RING.SF) / 2, size: 10 },
]
...
export default function RadialBracket({ matches, tz, hideScores }) {
  ...
  // QF winners join their two outer teams.
  for (const n of [...BRACKET.left.QF, ...BRACKET.right.QF]) {
    addBracket(n, RING.QF, LEAF_R, leafAngleOf[`${n}:0`], leafAngleOf[`${n}:1`], (RING.QF + LEAF_R) / 2)
  }
  // Each later round joins the two it feeds from, at that round's ring.
  const CHILD_RING = { SF: RING.QF }
  for (const round of ['SF']) {
    for (const n of [...BRACKET.left[round], ...BRACKET.right[round]]) {
      const [c1, c2] = childMatchNums(n)
  ...
  // Third-place play-off, shown just below the trophy: the two beaten
  // semi-finalists (its slots are "Loser Match 29/30", not winners, so it is fed
  // from decideMatch's loser rather than from the winner rings above).
  const THIRD_NUM = BRACKET.third[0]
  const [SF_LEFT, SF_RIGHT] = [BRACKET.left.SF[0], BRACKET.right.SF[0]]
  const thirdA = byNum[SF_LEFT] ? decideMatch(byNum[SF_LEFT])?.loser : null
  const thirdB = byNum[SF_RIGHT] ? decideMatch(byNum[SF_RIGHT])?.loser : null
  ...
  return (
    <div className={`radial-wrap${hasPath ? ' has-path' : ''}`}>
      ...
      <svg className="rb-svg" viewBox="0 0 1000 1080" role="img" aria-label="Knockout bracket, circular view">
        ...
        {/* Outer ring: the 8 quarter-finalists. */}
        {LEAF_SLOTS.map((s, i) => {
          const [x, y] = leafPos(s)
      ...
      <p className="rb-hint">
        Outer ring = quarter-finals. Each match’s winner advances one ring inward toward the trophy;
