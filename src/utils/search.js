...
// The venue table spells the host "United States"; searchers type all sorts of
// things. Match the synonym set against either spelling so "usa", "us" and
// "america" all land — the sibling viewers store it as "USA", and an alias table
// that assumes one spelling silently drops every match under the other.
const US_SYNONYMS = ['us', 'usa', 'united states', 'america', 'united states of america']
function matchCountry(country, v) {
  const c = country.toLowerCase()
  if (c.includes(v)) return true
  if (US_SYNONYMS.includes(c) && US_SYNONYMS.includes(v)) return true
  return false
}
