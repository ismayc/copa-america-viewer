...
const VIEWS = [
  ...
  // Scenarios + QF Outlook are group-stage tools; they're hidden once the group
  ...
  { id: 'outlook', label: '🔮 QF Outlook', groupStageOnly: true },
...
const GOAL_ALERTS_KEY = 'copa:goalAlerts'
...
export default function App() {
  ...
  //     parsed from copa.txt; see services/results.js.
  //   • ESPN (`live`) — best-effort live overlay (running score + clock), plus
  //     `history` for finals that have aged out of its rolling scoreboard.
  // The sibling viewers carry a third (TheSportsDB), which has no Copa América
  // data on its free tier — so the cross-check here confirms across two.
  ...
  const loadResults = useCallback(async () => {
    ...
    // Fetch both together; only OpenFootball gates the status bar — ESPN is
    // best-effort and never fails it.
  ...
  // Group → quarter-final slot each finishing position feeds into.
  ...
  // `filtered`). Applies as each round finishes (group stage, then quarter-finals, …).
    ...
    <div className="app">
      <header className="app-header">
        <div className="title-block">
          <h1>
            <span className="trophy">🏆</span> Copa América 2024
          </h1>
      ...
      <div className={`results-bar results-${resultsState}`}>
        ...
        <span className="results-text">
          ...
          {resultsState === 'ok' && finishedCount === 0 && 'No results yet — kickoff is June 20, 2024'}
        </span>
        {liveCount > 0 && (
      ...
      <footer className="app-footer">
        <p>
          Kickoff times convert automatically to your selected timezone; they were published in US
          Eastern, the timezone CONMEBOL ran the tournament on — the 14 host stadiums themselves
          span four US zones. Broadcast info is for the United States. Copa América 2024 finished on
        ...
        <p className="disclaimer">
          ...
          CONMEBOL. “Copa América”, team, broadcaster, and tournament names are trademarks of their
          respective owners. Schedule &amp; results data via{' '}
          ...
          <a href={LIVE_SOURCE.homepage} target="_blank" rel="noopener noreferrer">ESPN</a>, which
          also cross-checks the final scores.
        </p>
        <p className="credit">
          Created by{' '}
          ...
          ·{' '}
          <a
            href="https://github.com/ismayc/copa-america-viewer"
            target="_blank"
